...
async function startServer() {
    try {
        ...
        if (resultsToUpdate.length > 0) {
            console.log(`Backfilling teacher_id for ${resultsToUpdate.length} results...`);
        ...
        const adminsToUpdate = await Admin.findAll({ where: { role: 'sub_admin' } });
        if (adminsToUpdate.length > 0) {
            console.log(`Migrating ${adminsToUpdate.length} users from sub_admin to teacher...`);
        ...
        if (studentsWithTeacher.length > 0) {
            console.log(`Migrating ${studentsWithTeacher.length} student-teacher relationships to join table...`);
